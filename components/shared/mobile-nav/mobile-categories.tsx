...
const MobileCategories = ({ isOpen, onClose }: MobileCategoriesProps) => {
    ...
    // Remove the early return to allow AnimatePresence to handle exit animations
    // if (!isOpen) return null; Note: logic moved inside return
            ...
            {isOpen && (
                ...
                >
                    ...
                    <div className="flex flex-1 overflow-hidden pb-20">
                        ...
                        <div className="w-[70%] overflow-y-auto bg-white px-0">
                            {selectedCategory ? (
                                <div className="space-y-6">
                                    ...
                                    {selectedCategory.section && selectedCategory.section.length > 0 ? (
                                        <div className={cn(" gap-1", selectedCategory.section.some((sub) => sub.section && sub.section.length > 0) ? "flex flex-col" : "grid grid-cols-3")}>
                                            {selectedCategory.section.map((sub: SectionItem) => {
                                                ...
                                                return (
                                                    ...
                                                    <div key={sub.id} className="flex flex-col gap-2 justify-start w-full">
                                                        ...
                                                        {hasChildren && (
                                                            <div>
                                                                <Link
                                                                    href={subHref}
                                                                    onClick={onClose}
                                                                    className="flex flex-col gap-2 p-2 hover:bg-gray-50 bg-gray-200"
                                                                >
                                                                    <span className="text-xs font-medium line-clamp-2">{sub.title}</span>
                                                                </Link>
